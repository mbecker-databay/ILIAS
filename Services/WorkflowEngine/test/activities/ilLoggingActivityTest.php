...
class ilLoggingActivityTest extends PHPUnit_Framework_TestCase
{
	...
	{
		// Act
		$activity = new ilLoggingActivity($this->workflow);
	...
	public function testExecute()
	{
		// Arrange
		$activity = new ilLoggingActivity($this->node);
		$activity->setLogFile(vfsStream::url('example/log.txt'));
		...
		$fp = fopen(vfsStream::url('example/log.txt'), 'r');
		$line = fgets($fp);
	...
	public function testPassInUnwriteablePath()
	{
		// Arrange
		$activity = new ilLoggingActivity($this->node);
		$activity->setLogFile(vfsStream::url('example.txt'));
	}
